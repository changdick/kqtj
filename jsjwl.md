...
### 信道的多路复用
方法： 频分多路复用FDM、时分多路复用TDM、波分多路复用WDM、码分多路复用CDM。  
频分多路复用：信号调制成不同的频率，信道允许不同频率的共同传播。 不同用户用不同的频带传播。  
时分多路复用：切分时间片成小的时隙（时分复用帧），每个用户只能在自己的时隙里使用。  
波分多路复用：光的频分多路复用。  
**码分多路复用**：每个用户有自己的码片向量 $m_i$,要求是两两正交。用码片 $m_i$和负的码片 $-m_i$编码1和0。传播的时候，不同用户要传播的数据可以一起传播，传播的就是 $\sum k_i m_i$, 收到数据后， 拿对应用户的码片向量和数据内积，就能提取出该用户发来的数据，即 $(\sum k_im_i) \cdot m_j=k_jm_j^2$, 再除以 $m_j^2$就能够得到系数k是1还是-1了。  

### 调制方法
调制其实上是调成高频的波， 把信号藏到波函数的 振幅、频率、相位中表示。  
...
正交幅值调制QAM  同时改变幅值相位，不改变频率。  为什么？因为改变频率是要牺牲频带利用率的，2FSK的频带利用率低。QAM不改变频率，具有频带利用率高的优点。  
